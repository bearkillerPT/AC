...
unsigned short GraphCheckInvariants(Graph* g) {
  assert(ListGetSize(g->verticesList) == g->numVertices);
  int totalEdges = 0;
  unsigned int* neighbors;
  unsigned int inDegrees[g->numVertices];
  for(int i = 0; i < g->numVertices; i++) inDegrees[i] = 0;
  for(int i = 0; i < g->numVertices; i++) {
    neighbors = GraphGetAdjacentsTo(g, i);
    if(g->isDigraph) {
      assert(GraphGetVertexOutDegree(g, i) == neighbors[0]);
      for(int j = 1; j <= neighbors[0]; j++) {
        inDegrees[neighbors[j]] += 1;
        //printf("%d: %d in++\n", i, neighbors[j]);
      }
    }
    else {
