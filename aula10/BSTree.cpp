...
// Pre-order, In-order or Post-order?   A: In-order
...
static int _treeGetHeight(const struct _BSTreeNode* root) {
  ...
  int rightH = 1 + _treeGetHeight(root->right);
  int leftH = 1 + _treeGetHeight(root->left);
...
int BSTreeContains(const BSTree* header, const void* item) {
  ...
  int cmp;
  while (root != NULL) {
    cmp = header->compare(item, root->item);
    if(cmp == 0) return 1;
...
static void _treeTraverseINOrder(struct _BSTreeNode* root, void (*function)(void* p)) {
...
int BSTreeAdd(BSTree* header, void* item) {
  ...
  if(root == NULL) header->root = newNode;
  else if(cmp > 0) prev->right = newNode;
...
// R: Cada node teria de ter a altura de cada sub-arvore
//
// Q: E que funções teriam de ser modificadas?
// R: todas as que dependerem de _getNumNodes.
//
...
// R: Se cada node tivesse uma referência para o parent poder-se-ia chamar o _treeRemoveItem com o parent
//
...
assert(header != NULL);
  ...
  struct _BSTreeNode* pNode = _getPointerToKthNode(header->root, k);
  void* pItem = pNode->item;
  _treeRemoveItem(pNode->parent, pItem, header->compare);
  return pItem;
*/
//}
