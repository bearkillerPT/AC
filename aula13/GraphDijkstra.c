...
static void initDijkstra(GraphDijkstra* result) {
  unsigned int numVertices = GraphGetNumVertices(result->graph);
  for(int i = 0; i < numVertices; i++) {
    result->distance[i] = 999;
    result->predecessor[i] = -1;
  }
  result->distance[result->startVertex] = 0;
  PriorityQueue* queue = PriorityQueueCreate(numVertices, comparator, printer);
  ITEM items[numVertices];
  for(int i = 0; i < numVertices; i++){
    items[i].id = i;
    items[i].pri= result->distance[i];
    PriorityQueueInsert(queue, (void*) &items[i]); 
  }
  unsigned int* neighbors;
  int* nDistance;
  while(!PriorityQueueIsEmpty(queue)) {
    ITEM* item = (ITEM*) PriorityQueueGetMin(queue);
    PriorityQueueRemoveMin(queue);
    int v = item->id;
    if(item->pri == INT_MAX) {
      break;
    } 
    neighbors = GraphGetAdjacentsTo(result->graph, v);
    nDistance = GraphGetDistancesToAdjacents(result->graph, v);
    for(int i = 1; i <= neighbors[0]; i++) {
      if(result->distance[neighbors[i]] > result->distance[v] + nDistance[i]) {
        result->distance[neighbors[i]] = result->distance[v] + nDistance[i];
        result->predecessor[neighbors[i]] = v;
        items[neighbors[i]].pri = result->distance[neighbors[i]];
        PriorityQueueDecreasePriority(queue, &items[neighbors[i]]);
      }
    }
    free(neighbors);
    free(nDistance);
...
GraphDijkstra* GraphDijkstraExecute(Graph* g, unsigned int startVertex) {
  ...
  // result->predecessor
  //
  result->distance = (int* ) calloc(numVertices, sizeof(int));
  assert(result->distance != NULL);
  result->predecessor = (int* ) calloc(numVertices, sizeof(int));
